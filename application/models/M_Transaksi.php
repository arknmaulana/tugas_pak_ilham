...
class M_Transaksi extends CI_Model {
	...
	public function cekstok(){
		...
		for($i=0;$i<count($this->cart->contents());$i++){
			...
			$stok = $this->db->where('kode_sepatu', $this->input->post('kode_sepatu')[$i])->get('sepatu')->row()->stok;
	...
	public function simpan_db(){
			...
			$this->db->where('kode_sepatu', $this->input->post('kode_sepatu')[$i])->update('sepatu', $u_stok);
			}
			...
			$s_dttransaski[] = array('kode_transaksi' => $ts->kode_transaksi,
									 'kode_sepatu'=>$this->input->post('kode_sepatu')[$i],
		...
		public function ambil_dts($kode_transaksi){
			...
			return $this->db->join('sepatu', 'sepatu.kode_sepatu = detail_transaksi.kode_sepatu')
